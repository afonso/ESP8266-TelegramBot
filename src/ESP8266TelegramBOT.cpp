...
/***********************************************************************
 * sendDocument - function to send document to telegram                  *
 * (Arguments to pass: chat_id, document to transmit) *
 ***********************************************************************/
void TelegramBOT::sendDocument(String chat_id, String document)  {
...
/***********************************************************************
 * sendChatAction - function to send action to telegram                  *
 * (Arguments to pass: chat_id, action) *
 ***********************************************************************/
void TelegramBOT::sendChatAction(String chat_id, String action)  {
   ...
   // Serial.println("SEND Message ");
    ...
    if (action!="") {
      while (millis()<sttime+8000) {    // loop for a while to send the message
    String command="bot"+_token+"/sendChatAction?chat_id="+chat_id+"&action="+action;
    String mess=connectToTelegram(command);
    //Serial.println(mess);
    int messageLenght=mess.length();
    for (int m=5; m<messageLenght+1; m++)  {
